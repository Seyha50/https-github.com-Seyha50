...
class _ContactScreenState extends State<ContactScreen> {
  ...
  String _selectedFilter = 'All'; // 'All' | 'Recent' | 'Favourites'
  ...
  List<ContactModel> _applyFilters(List<ContactModel> contacts) {
    List<ContactModel> result = List.of(contacts);

    // Filter chip
    if (_selectedFilter == 'Favourites') {
      result = result.where((c) => c.isFavourite).toList();
    } else if (_selectedFilter == 'Recent') {
      // getContacts() already orders by createAt descending, so the
      // first N are the most recently added.
    ...
    if (_searchQuery.trim().isNotEmpty) {
      ...
      result = result.where((c) {
        ...
        return fullName.contains(query) || c.phone.contains(query);
      }).toList();
    }
  ...
  Future<void> _toggleFavourite(ContactModel contact) async {
    try {
    ...
    return Padding(
      ...
      child: ChoiceChip(
        ...
        labelStyle: TextStyle(
          color: isSelected ? AppColors.white : AppColors.bgColor,
          ...
          Expanded(
            child: StreamBuilder<List<ContactModel>>(
              ...
              builder: (context, snapshot) {
                ...
                final contacts = _applyFilters(snapshot.data ?? []);
                if (contacts.isEmpty) {
                  return const Center(child: Text('No contact found...'));
                }
                return ListView.builder(
                        ...
                        child: Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              // Favourite toggle
                              IconButton(
                                onPressed: () => _toggleFavourite(mycontact),
                                icon: Icon(
                                  mycontact.isFavourite
                                      ? Icons.star
                                      : Icons.star_border,
                                  color: mycontact.isFavourite
                                      ? Colors.amber
                                      : Colors.grey,
                                ),
                                tooltip: 'Favourite',
